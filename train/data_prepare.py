...
folders = ["slope", "wing", "ring"]
names = [
    "BEN3", "MAX2", "BRT2"  , "NOM", "BEN", "BEN2", "benji", "BENL",  "MAX"#, "BEN3_f", "MAX2_f"  , "NOM_f", "BRT2_f", "BEN_f", "BEN2_f", "benji_f", "BENL_f",  "MAX_f"
]
...
def generate_negative_data(data):  # pylint: disable=redefined-outer-name
  ...
  for i in range(100):
    ...
    start_x = (random.random() - 0.5) * 1000
    start_y = (random.random() - 0.5) * 1000
    start_z = (random.random() - 0.5) * 1000
    x_increase = (random.random() - 0.5) * 5
    y_increase = (random.random() - 0.5) * 5
    z_increase = (random.random() - 0.5) * 5
    for j in range(128):
      dic[DATA_NAME].append([
          start_x + j * x_increase + (random.random() - 0.5) * 3,
          start_y + j * y_increase + (random.random() - 0.5) * 3,
          start_z + j * z_increase + (random.random() - 0.5) * 3
      ])
    data.append(dic)
  # Random
  for i in range(100):
    if i > 80:
      ...
      dic = {DATA_NAME: [], LABEL_NAME: "negative", "name": "negative6"}
    for j in range(128):
      dic[DATA_NAME].append([(random.random() - 0.5) * 500,
  ...
  for i in range(100):
    ...
    for j in range(128):
      dic[DATA_NAME].append([
          start_x + (random.random() - 0.5) * 20,
          start_y + (random.random() - 0.5) * 20,
          start_z + (random.random() - 0.5) * 20
      ])
    data.append(dic)
